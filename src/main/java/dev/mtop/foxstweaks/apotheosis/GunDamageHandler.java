 ...
 *
 * <p>A TACZ gun's damage comes from its gun pack and only gains a few flat points from affixes, so
 * it never grows with rarity the way Ascension's melee gear does - and Ascension's mobs are tuned
 * for that. The multiplier runs from 1x up to {@code gunDamageMaxMultiplier} along
 * {@link RarityPatcher#CURVE}, the same curve the affix extrapolation uses.
 *
 * <p>Safe on a pack without Apotheosis or Ascension: no Apotheosis type appears here (the lookup
 * lives in {@link ApothicRarityLookup}) and it is not reached until both are known to be loaded.
 */
...
public final class GunDamageHandler {
    ...
    public static void onIncomingDamage(LivingIncomingDamageEvent event) {
        ...
        if (!(source.getEntity() instanceof Player player) || !(source.is(BULLETS) || source.is(ARMOR_PIERCING_PARTS)))
            return;
    ...
    private static boolean isAscensionLoaded() {
        if (loaded == null) {
            ...
            loaded = mods.isLoaded(FoxsTweaks.APOTHEOSIS) && mods.isLoaded("apothic_ascension");
        }
